...
def show_genre_country_films(genre):
    ...
    for i in genre_movies:
        deter_number += 1
    return render_template('genres details.html',movies = genre_movies[0:18],number = 0,countryName = countryName, genre = genre,deter_number = len(genre_movies[0:18]))
...
def show_nextpage(number,countryName, genre):
    ...
    if countryName == 'All':
        ...
        for i in genre_movies:
            deter_number += 1
        return render_template('genres details.html',movies = genre_movies[19*p:19*p+18],number = p,countryName = countryName,genre=genre,deter_number = len(genre_movies[19*p:19*p+18]))
    else:
        deter_number = 0
        country_all_movies = (Film.query.join(Country, Film.film_id == Country.film_id).filter_by(country=countryName)).join(Genre, Film.film_id == Genre.film_id).filter_by(genres=genre)
        for i in country_all_movies:
            deter_number += 1
        return render_template('genres details.html', movies=country_all_movies[19*p:19*p+18], number=p, countryName=countryName,genre = genre,deter_number = len(country_all_movies[19*p:19*p+18]))
...
def show_prevpage(number, countryName, genre):
    ...
    if countryName == 'All':
        genre_movies = Film.query.join(Genre, Film.film_id == Genre.film_id).filter_by(genres=genre)
        return render_template('genres details.html',movies = genre_movies[19*p:19*p+18],number = p,countryName = countryName,genre = genre)
    else:
        country_all_movies = (Film.query.join(Country, Film.film_id == Country.film_id).filter_by(country=countryName)).join(Genre, Film.film_id == Genre.film_id).filter_by(genres=genre)
...
def show_country_films(countryName,genre):
    ...
    for i in country_all_movies:
        deter_number += 1
    return render_template('genres details.html',movies = country_all_movies[0:18],number = 0, countryName = countryName,genre = genre,deter_number = len(country_all_movies[0:18]))
