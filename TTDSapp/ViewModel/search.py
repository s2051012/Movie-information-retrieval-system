...
#         self.stop_words = set()
#         # file = open(os.path.join(utility.__file__, 'englishST.txt'))
#         filepath = os.getcwd()
#         #print(filepath.join('/TTDSapp/ViewModel/englishST.txt'))
#         path = filepath + '/TTDSapp/ViewModel/englishST.txt'
#         print(path)
#         file = open(path)
#         for line in file:
...
class Searcher():
    ...
    def search_candidate_document(self, list1, list2, max_diff=3, weight=6.0, decay=1.0):
        """
        :param list1: list/tuple of tuples (primary key, document id, token, position)
        :param list2: list/tuple of tuples (primary key, document id, token, position)
        :param max_diff: max word distance we count
